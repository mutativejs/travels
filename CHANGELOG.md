...
- **Breaking:** Replace `getTravelPatchesValidationError` with `validateTravelPatches`, which validates and returns canonicalized patch groups in one step; the `allowNonJsonPathSegments` escape hatch is removed together with the durable patch-path contract.
